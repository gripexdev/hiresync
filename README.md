...
4. [Fonctionnalités implémentées](#4-fonctionnalités-implémentées)
   ...
   - [4.3 Optimisation IA (AI Gateway multi-fournisseurs)](#43-optimisation-ia-ai-gateway-multi-fournisseurs)
     ...
     - [4.3.3 Résilience : fallback multi-fournisseurs + gestion d'erreurs](#433-résilience--fallback-multi-fournisseurs--gestion-derreurs)
...
| 4 | Recevoir les **suggestions de l'IA** (passerelle multi-fournisseurs : Gemini → Groq → OpenRouter → local) en quelques secondes |
...
| IA / LLM | **AI Gateway** → Gemini 2.0 Flash · Groq Llama 3.3 70B · OpenRouter (`:free`) · Ollama local | Optimisation du CV avec fallback automatique |
| PDF parsing | **Apache PDFBox 3.0** | Extraction du texte depuis les CVs PDF |
| HTTP Client | **Spring WebFlux WebClient** | Appels HTTP vers les fournisseurs IA |
...
│   │   │   ├── service/        ← Logique métier (AtsScorer, AiGatewayService, ai/*Provider, CvService)
...
│   └── src/main/resources/
│       └── application.yml     ← Configuration (DB, RabbitMQ, JWT, clés Gemini/Groq/OpenRouter, Ollama)
│
...
### 4.3 Optimisation IA (AI Gateway multi-fournisseurs)

L'ancien `OpenRouterService` mono-fournisseur a été remplacé par une **passerelle IA (`AiGatewayService`)** qui essaie plusieurs fournisseurs gratuits dans l'ordre jusqu'à ce que l'un réponde. Objectif : un service **toujours disponible** même si un fournisseur est en panne ou a épuisé son quota.

**Architecture — interface `AiProvider` + 4 implémentations :**

```
AiGatewayService.optimizeCv()
        │
        ▼  essaie dans l'ordre, bascule au suivant en cas d'échec
   ┌──────────────────────────────────────────────────────────────┐
   │ 1. Gemini 2.0 Flash    (Google,  ~1 500 req/jour)             │
   │ 2. Groq Llama 3.3 70B  (Groq,    ~14 400 req/jour, le + rapide)│
   │ 3. OpenRouter          (modèles :free, backoff 429 interne)   │
   │ 4. Ollama local        (auto-hébergé, désactivé par défaut)   │
   └──────────────────────────────────────────────────────────────┘
        │
        ▼
   AiResult(content, provider)   ← on garde le NOM du fournisseur qui a réussi
```

Chaque fournisseur implémente `AiProvider` (`name()`, `isEnabled()`, `call(...)`). Le gateway les injecte dans l'ordre de priorité et renvoie un `AiResult(content, provider)` : le **nom réel du modèle utilisé** remonte jusqu'à la DB (`modelUsed`) et jusqu'à l'UI.

**Comment ça marche :**

```
Angular                  Spring Boot              AI Gateway
   │                         │                         │
   ...
   │  (WebSocket + polls GET /api/cv/optimize/:id /3s)  │
```

**Le prompt envoyé au modèle** demande un seul objet JSON `{atsKeywords, optimizedCv, suggestions}`. Le schéma `optimizedCv` couvre **toutes les sections du CV** — `summary`, `coreCompetencies`, `experience`, `projects`, `education`, `certifications`, `skills`, `languages` — et le prompt impose des **règles de complétude strictes** :

> PRESERVE EVERYTHING — inclure CHAQUE diplôme, CHAQUE langue (avec son niveau), CHAQUE projet (avec ses technologies réelles), CHAQUE expérience et TOUTES les compétences. On réécrit/réorganise, on ne supprime jamais une entrée factuelle.

Ces règles ont été ajoutées après avoir constaté que le modèle « résumait » le CV en supprimant les projets académiques, des diplômes et des langues. Le champ `projects` était absent du schéma : il a été ajouté, et `maxTokens` est passé de 2800 → **4096** pour éviter toute troncature du JSON.

**Normalisation des types de suggestion :** le LLM invente parfois des `type` hors énumération (`summary_rewritten`, `contact_info_added`). `OptimizationConsumer` les **normalise** vers les 4 valeurs canoniques (`keyword_added`, `section_rewritten`, `format_improved`, `skill_added`) avant stockage, et l'UI applique un fallback de style pour tout type inconnu — plus de libellé brut non stylé.

**Résultat réel observé (CV dev → poste Full Stack, via Groq) :**
> Score ATS : **61% → 82%** (+21 pts) en **~3 secondes**, compatibilité 85 %, 13/13 mots-clés couverts.  
> CV structuré complet : 4 diplômes, 4 projets (avec stack), 3 langues avec niveau, 13 compétences.

---

#### 4.3.1 Vérification de compatibilité profil ↔ offre

Avant de lancer la réécriture du CV, le pipeline effectue un **contrôle de compatibilité** entre le profil détecté dans le CV et le poste ciblé. Ce contrôle utilise un second appel LLM dédié (`AiGatewayService.assessCompatibility`), qui passe lui aussi par la chaîne de fallback multi-fournisseurs.
...
[Message RabbitMQ reçu]
        │
        ▼
AiGatewayService.assessCompatibility(cvText, jobTitle, jobDescription)
        │
...
1. La réponse d'optimisation contient un tableau `atsKeywords` — les mots-clés techniques et compétences que le LLM a extraits de l'offre ciblée (mêmes termes exacts que l'offre).
2. `AtsScorer.computeJobMatch(text, atsKeywords)` calcule, pour le CV original **et** le CV optimisé, combien de ces mots-clés sont couverts → un avant/après honnête et explicable.
3. Les listes `matched` / `missing` alimentent directement les champs `matchedKeywords` / `missingKeywords` de la réponse.
...
#### 4.3.3 Résilience : fallback multi-fournisseurs + gestion d'erreurs

La disponibilité repose sur **deux niveaux** de tolérance aux pannes :

**1. Fallback entre fournisseurs (`AiGatewayService.route`)** — chaque fournisseur est essayé dans l'ordre ; à la moindre exception, on passe au suivant et on accumule les erreurs pour le log :

```
Gemini 2.0 Flash ──(429 / erreur)──► Groq Llama 3.3 70B ──(échec)──►
OpenRouter (:free) ──(échec)──► Ollama local ──(tous échouent)──► RuntimeException
```

**2. Gestion d'erreurs par fournisseur** — chaque implémentation `AiProvider` traduit toute panne réseau ou HTTP en exception claire, pour que rien ne remonte à l'utilisateur sous forme de stack trace :

| Cas | Traitement |
|-----|------------|
| HTTP **429** | Détecté (quota épuisé) → bascule fournisseur suivant |
| HTTP **401/403** | Clé API invalide → message explicite, fournisseur suivant |
| **Connexion refusée / timeout** (`WebClientRequestException`) | Ex. Ollama non démarré → message clair, fournisseur suivant |
| **Réponse vide / JSON malformé / contenu null** | Levée d'exception → fournisseur suivant |
| **Gemini safety block** (`finishReason: SAFETY`) | Traité comme un échec → fournisseur suivant |

OpenRouter conserve en plus un **backoff interne sur 429** (les modèles `:free` partagent le même quota par clé, donc on attend plutôt que de cycler les modèles).

Si **tous** les fournisseurs échouent, le statut passe en `FAILED` et l'UI affiche : *"Tous les services IA sont temporairement indisponibles. Réessayez dans quelques minutes."*

**Affichage dynamique du modèle :** le nom du fournisseur qui a réellement traité la demande est poussé via WebSocket (champ `provider`) et stocké en DB (`modelUsed`). La page de chargement et la page de résultat affichent donc le **vrai modèle utilisé** (ex. `Groq Llama 3.3 70B`) au lieu d'un nom codé en dur.

**Endpoints d'administration (`AdminAiController`)** pour diagnostiquer la chaîne :
- `GET /api/admin/ai/providers` — liste les fournisseurs et leur statut activé/désactivé
- `POST /api/admin/ai/test` — envoie un prompt de test, avec une liste `skip` pour forcer le contournement de fournisseurs et vérifier chaque maillon du fallback
...
- `OptimizationConsumer.java` — écoute la queue, appelle l'AI Gateway, met à jour la DB, pousse le WebSocket

**Pourquoi asynchrone ?**  
L'appel IA peut prendre de quelques secondes à plusieurs dizaines de secondes (selon le fournisseur et les fallbacks). Si c'était synchrone, l'utilisateur attendrait avec une requête HTTP bloquée. Avec RabbitMQ :
...
**Dead Letter Queue :**  
Si le consumer échoue 3 fois (tous les fournisseurs IA en échec, timeout…), le message est automatiquement routé vers `cv.optimize.dlq` pour inspection manuelle (visible dans l'UI RabbitMQ : `http://localhost:15672`).
...
Mode 1 — Nouvelle optimisation (depuis "Optimiser IA")
  ...
  → _startProcessing() → animation IA → RabbitMQ → AI Gateway → WebSocket (+ nom du modèle) → résultat
...
Si le CV uploadé est mal parsé (texte brut non structuré), l'utilisateur peut cliquer "Rebuild with AI" — le studio envoie le texte brut à l'AI Gateway qui retourne un JSON structuré (`{name, email, experience[], projects[], skills[], education[]}`) pour préremplir automatiquement tous les champs.
...
│   ├── service/
│   │   ├── AtsScorer.java                   PDFBox extraction + scoring ATS générique + computeJobMatch() (mots-clés offre)
│   │   ├── AiGatewayService.java            Passerelle IA : prompts, parsing, routing + fallback entre fournisseurs (optimizeCv, assessCompatibility)
│   │   ├── ai/AiProvider.java               Interface fournisseur (name, isEnabled, call)
│   │   ├── ai/GeminiProvider.java           Gemini 2.0 Flash + gestion d'erreurs (429/401/safety)
│   │   ├── ai/GroqProvider.java             Groq Llama 3.3 70B + gestion d'erreurs
│   │   ├── ai/OpenRouterProvider.java       OpenRouter (:free) + backoff 429 interne
│   │   ├── ai/LocalModelProvider.java       Ollama local (désactivé par défaut)
...
│   │   └── OptimizationConsumer.java        @RabbitListener → assessCompatibility → AI Gateway → normalisation suggestions → AtsScorer
│   ├── dto/
